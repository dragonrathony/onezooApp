...
// var SERVER_PATH     = "http://gloveman.onezoo.site/";
// var SERVER_API_PATH = "http://gloveman.onezoo.site/webplanex/";
var customerID = '';
var supplierID = 0;
...
function sendSmsMessage(number, message, successCallback, errorCallback) {
    ...
    var success = function () {
        console.log('Message sent successfully ', number);
    };
...
function onDeviceReady() {
    ...
    $(document).on('tap', '.plus-button-click', function () {
        ...
        getDeviceContacts(function (data) {
            ...
            $.mobile.changePage("#invite_supplier_contacts", {
                transition: "none"
            });
    ...
    $(document).on('tap', '#request_pincode', function () {
        db.query('SELECT * FROM customers', function (tx, results) {
            ...
            if (len > 0) {
                ...
                $.mobile.changePage("#forgot_password_page", {
                    transition: "none"
                });
    ...
    $(document).on("pagebeforeshow", "#quick_order_page", function () {
        // console.log("document on quick order clicked!!!!");
        goToQuickOrderPage(supplierID);
    });
    $(document).on("pageshow", "#supplier_home_page, #suppliers_page, #quick_order_page", function () {
        // console.log("document on quick order clicked!!!!");
        chat_notification_interval = setInterval(chatNotificationCount, 5000);
    });
    $(document).on("pagebeforehide", "#supplier_home_page, #suppliers_page, #quick_order_page, #home_page", function () {
        // console.log("document on quick order clicked!!!!");
        clearInterval(chat_notification_interval);
    });
    $(document).on('tap', 'ul.dynamic_quick_order_data li a.custom_list_a', function () {
        var activePage = $.mobile.activePage.attr('id'); // = quick_order_page
        var newsupplierID = $(this).data('supplier_id'); // = 5
        $('#' + activePage + ' ul.dynamic_quick_order_data li ul').not(this).each(function (index) {
            // console.log("//////////////");
            //     console.log($(this));
            if (!$(this).hasClass('supplier_' + newsupplierID) && $(this).css('display') != 'none') {
        ...
        $(this).siblings('.custom_content').slideToggle();
        $(this).find('i').toggleClass('arrow-up-img arrow-down-img');
        return false;
    });
    ...
    $(document).on('tap', '.sidebarmenu', function () {
        ...
        if (activePage == page_ID && supplierID != newsupplierID) {
            ...
            if (page_ID != 'supplier_home_page') {
                $.mobile.changePage("#supplier_home_page", {
                    transition: "none"
                });
    ...
    if (device.platform != "browser") {
        var push = PushNotification.init({
            "android": {
                "senderID": "657980447754",
                "forceShow": "true",
                "icon": "www/img/push_icon.png"
            },
            "ios": {
                "alert": "true",
                "badge": "true",
...
function genrateViewfetchSidebar(dataresult) {
    // console.log("here is generateViewfetchsidebar from fetsidebar")
    ...
    // console.log("goto generate_sidebar from generateviewfetchsidebar")
    generate_sidebar();
...
function fetchsidebar(reset_sidebar) {
    // console.log('here is fetch side bar')
    ...
    if (sidebar_supplier_session && sidebar_supplier_session != null) {
        // console.log("here is sidebar supplire session........");
        // console.log(sidebar_supplier_session);
        // console.log("goto generateviewfetchsidebar from sidebarsuppliersession");
        genrateViewfetchSidebar(sidebar_supplier_session);
    }
    db.query('SELECT * FROM customers', function (tx, results) {
        // console.log("here is * from customer")
        var len = results.rows.length;
        if (len > 0) {
            var row = results.rows.item(0);
            customers_id = row.customers_id;
            // console.log(" len more than 0")
            ...
            $.ajax({
                ...
                success: function (dataresult) {
                    ...
                    if (JSON.stringify(sidebar_supplier_session) != JSON.stringify(dataresult)) {
                        // console.log("goto generateviewfetchsidebar from dbquery...");
...
function generate_sidebar() {
    // console.log("here is generate_sidebar");
    win_height = $(window).height();
    temp = win_height - 80;
    $('.sidebar-main-content').css({
        'height': temp,
    ...
    if (sidebar_supplier.length > 0) {
        for (var i = 0; i < sidebar_supplier.length; i++) {
            ...
            golbal_side_menu_sub += '<li onClick="getQuickOrders(' + sidebar_supplier[i].supplier_id + ');" data-supplier_id="' + sidebar_supplier[i].supplier_id + '" class="sidebarmenu first_page"><a href="#">Quick Order</a></li>';
    ...
    if (typeof dataresult.products_list != "undefined" && dataresult.products_list.length > 0) {
        for (var i = 0; i < dataresult.products_list.length; i++) {
            ...
            if (active_supplier_option && supplierID != 'all_saved_suppliers') {
                ...
                var addtoCart_icon = '<a data-role="none" href="javascript:addCart(' + products_id + ',' + products_suppl_id + ',' + null + ',' + null + ',' + true + ')" class="add_to_cart" style="color:#fff !important;"><i class="fa fa-cart-plus fa-2x"></i></a>';
...
function userValidate() {
    ...
    // cordova.plugin.http.post(CONECTION_SERVER(), formData, {}, function (response) {
    //     console.log(response);
    //     // prints 200
    //     var dataresult;
    //     if (typeof (response.data) === 'string') {
    //         dataresult = JSON.parse(response.data);
    //     } else {
        ...
        $.ajax({
            ...
            success: function (dataresult) {
                ...
                if (dataresult.result == "error") {
                    ...
                    $.mobile.changePage("#reset_password_page", {
                        transition: "none"
                    });
    ...
    $.ajax({
        ...
        success: function (dataresult) {
            ...
            if (dataresult.result == "error") {
                ...
                countdown();
                playTimer();
                $.mobile.changePage("#reset_pincode_page", {
                    transition: "none"
                });
        ...
        success: function (dataresult) {
            ...
            if (dataresult.result == "error") {
                ...
                getSingleColumn("products_id", "products_list", " WHERE products_id = " + id, function (data) {
                    ...
                    $("#after" + id).append('<div class="action_div">' + add_to_list_html + '<a data-product-id="' + id + '" data-package-id="' + pro_supplier_id + '" href="javascript:addCart(' + id + ',' + pro_supplier_id + ',' + null + ',' + null + ',' + true + ')" class="add_to_cart">Add To Cart</a><div style="float:right;">' + product_qty + '</div></div><div class="clear_both"></div>');
        ...
        if (len > 0) {
            ...
            $.ajax({
                ...
                success: function (dataresult) {
                    ...
                    if (window.localStorage.getItem('showSideMenu') != 'true') {
                        $(".nav-panel-main").panel("close");
                    }
...
function actionAddressBook(address_book_id, customers_id) {
    ...
    window.plugins.actionsheet.show(options, function (buttonIndex) {
        setTimeout(function () {
            ...
            window.plugins.actionsheet.hide({}, function () {
                deleteValue("action_address_book_id");
                deleteValue("action_customers_id");
            }, function () {});
        });
...
function quickCheckout(supplier_id) {
    ...
    getSingleColumn("customers_id", "customers", "WHERE customers_id > 0", function (data) {
        db.query('SELECT * FROM customers_basket WHERE supplier_id=' + supplierID + ' AND customers_id=' + data.customers_id + ' ORDER BY customers_basket_id DESC', function (tx, results) {
            var len = results.rows.length;
            var custom_listview_qty = $(".input_products_list_number").filter(function () {
                return $(this).val();
            }).length;
            if (len > 0) {
                goTO('checkout_page');
